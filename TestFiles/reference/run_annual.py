...
print recipe_json["analysis_grids"][0].keys()
if "da_threshold" in recipe_json["analysis_grids"][0].keys():
    da_threshold = recipe_json["analysis_grids"][0]["da_threshold"]
else:
    da_threshold = None
if "udi_min_max" in recipe_json["analysis_grids"][0].keys():
    udi_min_max = recipe_json["analysis_grids"][0]["udi_min_max"]
else:
    udi_min_max = None
if "occ_schedule" in recipe_json["analysis_grids"][0].keys():
    occ_schedule = recipe_json["analysis_grids"][0]["occ_schedule"]
else:
    occ_schedule = None
