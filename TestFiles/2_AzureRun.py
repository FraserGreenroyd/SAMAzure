...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=os.path.join('resources', 'radiance_case'))  # TODO - Remove line post testing
    parser.add_argument(
        "-c",
        "--configFile",
        type=str,
        help="Path to the azure config file",
        default=os.path.join('common', 'configuration.cfg'))  # TODO - Remove line post testing
    parser.add_argument(
        "-id",
        "--projectID",
        type=str,
        help="ID for the job being undertaken",
        default="000000-testproject-3513")  # TODO - Remove line post testing
    parser.add_argument(
        "-dp",
        "--deletePool",
        type=str,
    ...
    container_sas_token = block_blob_client.generate_container_shared_access_signature(project_id, permission=azureblob.ContainerPermissions(read=True, write=True), expiry=datetime.datetime.utcnow() + datetime.timedelta(minutes=30), )
    ...
    # TODO: For some reson with >100 tasks two pools of 100 were created, maybe we just cerate a set of jobs and assign to the same pool?
    # A chunked list of grid indices - to create multiple pool instances
    job_chunks = list(common.helpers.chunks(list(range(0, len(analysis_grid_names))), 2)) # TODO - change new pool amount
                    ...
                    batchmodels.OutputFile(
                        ...
                        destination=batchmodels.OutputFileDestination(
                            container=batchmodels.OutputFileBlobContainerDestination(
                                container_url="https://{0:}.blob.core.windows.net/{1:}?{3:}".format(storage_account_name, project_id, os.path.join("Results", output_file), container_sas_token))),
    ...
    # TODO: Add a wait for all jobs to be complete - not just the last
